...
function TechnicalSkillsMultichoice({ value, onChange, jobtypeId }) {
    ...
    useEffect(() => {
        ...
        if (jobtypeId) {
            api.get(`/api/technical-skills/?jobtype=${jobtypeId}`)
                .then(response => setTechnicalSkills(response.data))
                .catch(error => console.error('Error fetching technical skills:', error));
        } else {
            setTechnicalSkills([]); // Clear skills when no jobtype is selected
        }
    }, [jobtypeId]);

    return (
        <div className="soft-skills-container">
            <h1 className="soft-skills-title">Select technical skills you have been trained on and the mode of delivery</h1>
